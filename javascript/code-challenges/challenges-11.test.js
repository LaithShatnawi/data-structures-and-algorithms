...
const divisibleByFiveTwoToThePower = (input) => {
  var filteredArr = input.map(function (subArr) {
    return subArr.filter(function (el) {
      return typeof el === 'number' && el % 5 === 0;
    });
  });

  var result = filteredArr.map(function (subArr) {
    return subArr.map(function (el) {
...
let findMaleAndFemale = (data) => {
  ...
  let boolean = true;
  data.map((el) => {
    if ((el.gender === 'male' || el.gender === 'female') && !boolean) {
      str = str + ` and ${el.name}`;
    }
    if ((el.gender === 'male' || el.gender === 'female') && boolean) {
      str = str + `${el.name}`;
...
let findShortest = (data) => {
  var shortestCharacter = starWarsData
    .filter(function(character) {
      return character.height !== 'unknown';
    })
    .reduce(function(shortest, currentCharacter) {
      var shortestHeight = parseInt(shortest.height);
      var currentHeight = parseInt(currentCharacter.height);
      return currentHeight < shortestHeight ? currentCharacter : shortest;
    });
